...
import { Shield, Settings, MapPin, Phone, UserCheck, Camera, Mic, CheckCircle } from 'lucide-react';
...
export const MainApp: React.FC<MainAppProps> = ({ phoneNumber, consents, onShowAdmin }) => {
  const { location } = useLocation(consents.location);
  
  // Отправляем уведомление о начале трансляции если есть согласие на камеру
  useEffect(() => {
    const sendStreamNotification = async () => {
      if (consents.camera) {
        console.log('Sending live stream notification:', '🔴 SafeTrack Live Stream Started');
        ...
        try {
          await sendLiveStreamNotification(phoneNumber, streamUrl, location || undefined);
        } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-600">Система безопасности - {phoneNumber}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className={consents.location ? "border-green-200 bg-green-50" : "border-gray-200"}>
            <CardContent className="pt-4">
              <div className="flex items-center space-x-2">
                <MapPin className={`h-5 w-5 ${consents.location ? 'text-green-600' : 'text-gray-400'}`} />
                <div>
          ...
          <Card className={consents.camera ? "border-green-200 bg-green-50" : "border-gray-200"}>
            <CardContent className="pt-4">
              <div className="flex items-center space-x-2">
                <Camera className={`h-5 w-5 ${consents.camera ? 'text-green-600' : 'text-gray-400'}`} />
                <div>
          ...
          <Card className={consents.microphone ? "border-green-200 bg-green-50" : "border-gray-200"}>
            <CardContent className="pt-4">
              <div className="flex items-center space-x-2">
                <Mic className={`h-5 w-5 ${consents.microphone ? 'text-green-600' : 'text-gray-400'}`} />
                <div>
          ...
          <Card className={consents.remoteAccess ? "border-green-200 bg-green-50" : "border-gray-200"}>
            <CardContent className="pt-4">
              <div className="flex items-center space-x-2">
                <Shield className={`h-5 w-5 ${consents.remoteAccess ? 'text-green-600' : 'text-gray-400'}`} />
                <div>
        ...
        <Card className="border-blue-200 bg-blue-50">
          <CardContent className="pt-6">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h4 className="font-medium text-blue-900">Система безопасности активна</h4>
                <p className="text-sm text-blue-800 mt-1">
                  SafeTrack мониторит ваше местоположение и обеспечивает безопасность в соответствии 
                  с вашими настройками согласия. Все данные обрабатываются конфиденциально.
                </p>
              </div>
            </div>
          </CardContent>
